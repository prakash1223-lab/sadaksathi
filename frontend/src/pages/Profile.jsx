...
// ── Report Card ───────────────────────────────────────────────────────────────
function ReportCard({ report, currentUserId, confirmDelete, setConfirmDelete, deleting, onDelete }) {
  ...
  const canDelete = report.reporter_id === currentUserId && !isFixed
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <p className="text-sm font-semibold text-gray-900 leading-snug flex-1">{report.title}</p>
            <div className="flex items-center gap-1.5 shrink-0">
              <span className={`text-xs px-2.5 py-0.5 rounded-full font-medium ${STA_STYLE[report.status] || STA_STYLE.pending}`}>
                {isFixed ? (
          ...
          {confirmDelete === report.id && (
            <div className="mt-2 p-3 bg-red-50 border border-red-100 rounded-xl">
              <p className="text-xs text-red-700 font-medium mb-2">🗑️ Delete this report? This cannot be undone.</p>
              <div className="flex gap-2">
                <button
                  ...
                  {deleting === report.id ? 'Deleting…' : 'Yes, delete'}
                </button>
                <button
                  onClick={() => setConfirmDelete(null)}
                  className="text-xs border border-gray-200 text-gray-600 font-semibold px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
              </div>
            </div>
...
export default function Profile() {
  ...
  const handleDeleteReport = async (report) => {
    ...
    try {
      ...
      setData(d => ({
        ...d,
        reports: d.reports.filter(r => r.id !== report.id),
        stats: {
          ...d.stats,
          total_reports: d.stats.total_reports - 1,
          pending:     d.stats.pending     - (report.status === 'pending'     ? 1 : 0),
          in_progress: d.stats.in_progress - (report.status === 'in_progress' ? 1 : 0),
        },
      }))
      setConfirmDelete(null)
      setToast('Report deleted successfully')
    } catch (err) {
      setToast(err.response?.data?.detail || 'Failed to delete report')
      setConfirmDelete(null)
